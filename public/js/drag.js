...
Drag.prototype.render = function() {
	...
	var stack1 = $('<div style="position:absolute;top:-4px;left:-4px;width:8px;height:8px;background:black" class="lt"></div>'),
		stack2 = $('<div style="position:absolute;top:-4px;right:-4px;width:8px;height:8px;background:black" class="rt"></div>'),
		stack3 = $('<div style="position:absolute;bottom:-4px;left:-4px;width:8px;height:8px;background:black" class="lb"></div>'),
		stack4 = $('<div style="position:absolute;bottom:-4px;right:-4px;width:8px;height:8px;background:black" class="rb"></div>');
	if (($('.lt').length + $('.rt').length + $('.lb').length + $('.rb').length) == 0) {
			...
			if(parseInt(targetLeft)<0){
				...
				if(targetWidth+parseInt(targetLeft)>mainWidth){
					if(targetWidth<mainWidth){
						...
						sel.css({width:targetWidth}).css('left',mainWidth-targetWidth)
					}else{
						that.css('width',mainWidth).css('left',0)
			...
			if(parseInt(targetTop)<0){
				...
				if(targetHeight+parseInt(targetTop)>mainHeight){
					if(targetHeight<mainHeight){
						...
						sel.css({height:targetHeight}).css('top',mainHeight-targetHeight)
					}else{
						that.css('height',mainHeight).css('top',0)
